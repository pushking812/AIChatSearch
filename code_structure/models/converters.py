...
def code_node_to_old_node(code_node: CodeNode) -> Node:
    ...
    if isinstance(code_node, ModuleNode):
        ...
        old_node = OldFunctionNode(name=code_node.name, args=code_node.signature.strip('()'), lineno_start=code_node.start_line, lineno_end=code_node.end_line)
    elif isinstance(code_node, MethodNode):
        old_node = OldMethodNode(name=code_node.name, args=code_node.signature.strip('()'), lineno_start=code_node.start_line, lineno_end=code_node.end_line)
    elif isinstance(code_node, CodeBlockNode):
        old_node = OldCodeBlockNode(name=code_node.name, line_range=f"{code_node.start_line}-{code_node.end_line}", lineno_start=code_node.start_line, lineno_end=code_node.end_line)
    else:
        # Для комментариев и импортов создаём простой узел
        old_node = Node(name=code_node.name, node_type=code_node.node_type, lineno_start=code_node.start_line, lineno_end=code_node.end_line)
    # Рекурсивно преобразуем детей
    for child in code_node.children:
        old_node.add_child(code_node_to_old_node(child))
    return old_node
...
def block_to_old_block_info(block: Block) -> MessageBlockInfo:
    ...
    # Создаём MessageBlock с индексом (используем block.block_idx)
    ...
    if block.code_tree:
        old.tree = code_node_to_old_node(block.code_tree)
    return old